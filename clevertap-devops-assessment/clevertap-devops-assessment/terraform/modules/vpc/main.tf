...
resource "aws_flow_log" "this" {
  ...
  # Parquet + Hive partitions: reduces Athena scan cost by ~80% vs plain text.
  # Query pattern: SELECT * FROM vpc_flow_logs WHERE year='2024' AND month='06'
  # AND srcaddr NOT IN (known internal CIDRs) AND action='REJECT'
  # Athena table is created separately (see terraform/modules/vpc/athena.tf)
  destination_options {
    file_format                = "parquet"
